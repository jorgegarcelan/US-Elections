    ...
    <main className="editorial-page">
      ...
      <section className="home-hero">
        ...
        <div className="hero-board" aria-label="2024 electoral result">
          <div className="hero-result hero-result-gop"><span className="hero-party">Republican</span><strong>309</strong><span>electoral votes</span></div>
          <div className="hero-threshold"><span>270</span><span>to win</span></div>
          <div className="hero-result hero-result-dem"><span className="hero-party">Democrat</span><strong>226</strong><span>electoral votes</span></div>
          <div className="electoral-stripe" aria-hidden="true">
            {Array.from({ length: 36 }, (_, index) => <span key={index} className={index < 20 ? "is-gop" : "is-dem"} />)}
          </div>
        </div>
      ...
      <section className="statement-section">
        ...
        <dl className="fact-grid">
          <div><dt>3,107</dt><dd>counties analysed</dd></div><div><dt>03</dt><dd>election cycles</dd></div>
          <div><dt>30+</dt><dd>census variables</dd></div><div><dt>538</dt><dd>electoral votes</dd></div>
        </dl>
      </section>
      ...
      <section className="timeline-section">
        ...
        <div className="election-timeline">
          {electionYears.map((election, index) => (
            <article key={election.year} className="timeline-card"><span className="timeline-number">0{index + 1}</span><h3>{election.year}</h3><p>{election.note}</p><div className="mini-result"><span className="dem-text">D {election.dem}</span><span className="gop-text">R {election.gop}</span></div></article>
          ))}
        </div>
